...
def main():
    ...
    parser_log.add_argument('-r', '--reverse', action='store_true', help="Reverses the order of the block entries to show the most recent entries first", required=False)
    ...
    if option == 'checkout':
        ...
        bc_obj.remove(item_id, reason, owner)
        sys.exit(0)
        ...
        case_id = args.c            # String, need to conv to UUID
        # Send off to log function
        bc_obj.log(reverse, num_entries, item_id, case_id)
        sys.exit(0)
    else:
        parser.print_help()
        sys.exit(587)
